...
// The assembled robot routes the physical joints opposite to the author's
// wiring table.
constexpr uint8_t LOWER_SERVO_PIN = 32;
constexpr uint8_t UPPER_SERVO_PIN = 26;

int ang1 = 0, offset1 = 0, pulse1 = 1000;
int ang2 = 0, offset2 = 0, pulse2 = 1000;
int servo2HomeTrimDeg = 0;
...
const int SERVO_COMMAND_MIN_DEG = 15;
const int SERVO_COMMAND_MAX_DEG = 165;
...
int lowerServoCommandDeg(int targetDeg) {
  return (int)clampf(targetDeg + 90,
                     SERVO_COMMAND_MIN_DEG, SERVO_COMMAND_MAX_DEG);
}

int upperServoCommandDeg(int targetDeg, int homeTrimDeg = 0) {
  return (int)clampf(-targetDeg + 90 + homeTrimDeg,
                     SERVO_COMMAND_MIN_DEG, SERVO_COMMAND_MAX_DEG);
}
...
void writeServoTargetsRad(float target1, float target2) {
  ...
  servo1.write(lowerServoCommandDeg(deg1));
  servo2.write(upperServoCommandDeg(deg2, servo2HomeTrimDeg));
}
...
void offset1M() {
  if(offset1 >= -300){
    ...
    servo1.detach();
    servo1.attach(LOWER_SERVO_PIN, 1500 + offset1 - pulse1, 1500 + offset1 + pulse1);
  }
  handleRoot();
}
void offset1P() {
...
void offset2M() {
  if(offset2 >= -300){
    ...
    servo2.detach();
    servo2.attach(UPPER_SERVO_PIN, 1500 + offset2 - pulse2, 1500 + offset2 + pulse2);
  }
  handleRoot();
}
void offset2P() {
...
void setup(){
  ...
  servo2HomeTrimDeg = preferences.getInt("upperhome", 0);
...
void loop(){
  ...
  */
  ...
  if (robotMode == MODE_GETUP) {
    ...
    servo1.write(lowerServoCommandDeg(ang1));
    servo2.write(upperServoCommandDeg(ang2, servo2HomeTrimDeg));
    delay(10);
  }
